const service = require("../services/ventaDetalle.service");
...
const listar = async (req, res) => {
  try {
    ...
    res.status(500).json({ message: "Error al listar detalle de ventas", error: error.message });
  }
};
...
const eliminar = async (req, res) => {
  try {
    ...
    res.status(400).json({ message: "Error al eliminar detalle de venta", error: error.message });
  }
};
